...
output_dir = current_file.parent.parent / "outputs" / "generation"
...
all_cell_types = sorted(cell_types)

ref_data=[]
for idx, celltype in enumerate(all_cell_types):
            matched_sentences = []
            ...
            sampled = random.sample(matched_sentences, 1)
            ref_data.append(sampled)
...
for output_file in output_dir.glob("*.jsonl"):
    ...
    with open(output_file, "r", encoding="utf-8") as pred_file:
        for line in pred_file:
            ...
            # Extract predicted content using regex.
            ...
            if match:
                ...
                predicted_cell = response
            predict_list.append(predicted_cell)
    ...
    # Save as JSON
    output_result_path = current_file.parent / "generation" / f"{model_name}.json"
