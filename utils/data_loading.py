...
def match_genes_to_scgpt_vocab(vocab_file, pert_data, logger, special_tokens):
    ...
    pert_data.var["id_in_vocab"] = [
        1 if gene in vocab else -1 for gene in pert_data.var["gene_name"]
    ]
    gene_ids_in_vocab = np.array(pert_data.var["id_in_vocab"])
    if logger:
        print(
    ...
    dataset_genes = pert_data.var["gene_name"].tolist()
